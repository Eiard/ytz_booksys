...
(
    ...
    overdue      boolean default 0, # 逾期状态位 默认未逾期0 超时更新为1
    isReturn     boolean default 0  # 是否还书状态位 未还书默认为0 还书后更新为1
);
...
# 判断是否逾期
# 事件每隔 1 分钟触发
# 状态位isReturn没还 处于之前为未逾期状态 当前时间逾期
CREATE event if not exists change_BorrowOverdue
